          ...
          <>
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-0 right-0 z-10 h-9 w-9 flex items-center justify-center disabled:opacity-60"
                aria-label={
                  isInWishlist ? "Remove from wishlist" : "Add to wishlist"
                }
              >
                <Heart
                  className={`h-4 w-4 md:h-6 md:w-6 ${isInWishlist ? "fill-black text-black" : "text-black"}`}
                />
              </button>
            </div>
                ...
                {productImages.map((image, index) => (
                  <button
                    ...
                    className={`h-19.5 w-16 shrink-0 border ${
                      selectedImage === image ? "border-black" : "border-0"
              ...
              <div className="px-6 mt-6 flex flex-col gap-3">
                ...
                <div className="flex gap-4 overflow-x-auto overflow-y-hidden hide-scrollbar">
                  {relatedProducts.slice(0, 6).map((item) => (
                    <ProductFrame
                      key={item.id}
                      price={formatFromBase(item.price)}
