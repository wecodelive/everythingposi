...
export default function AdminSettings() {
      ...
      setSettingsData({
        ...
        notificationsEnabled:
          typeof payload.settings?.notificationsEnabled === "boolean"
            ...
            : true,
        exchangeRates: {
          USD: payload.settings?.exchangeRates?.USD?.toString() || "",
          GBP: payload.settings?.exchangeRates?.GBP?.toString() || "",
          EUR: payload.settings?.exchangeRates?.EUR?.toString() || "",
        },
      });
    } catch (error) {
  ...
  const handleRateChange = (e) => {
    ...
    setSettingsData((prev) => ({
      ...prev,
      exchangeRates: { ...prev.exchangeRates, [name]: value },
    }));
  };
        ...
        <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Input
              id="rateUSD"
              name="USD"
              placeholder="USD rate (per base)"
              value={settingsData.exchangeRates.USD}
            ...
            <Input
              id="rateGBP"
              name="GBP"
              placeholder="GBP rate (per base)"
              value={settingsData.exchangeRates.GBP}
              onChange={handleRateChange}
              type="number"
            />
            <Input
              id="rateEUR"
              name="EUR"
              placeholder="EUR rate (per base)"
              value={settingsData.exchangeRates.EUR}
              onChange={handleRateChange}
              type="number"
            />
          </div>
